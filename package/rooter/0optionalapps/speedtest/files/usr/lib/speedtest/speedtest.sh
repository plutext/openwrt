...
#                Alternate servers are netperf-east.bufferbloat.net (east coast US),
...
END {
	...
	printf("  Latency:  (in msec, %d pings, %4.2f%% packet loss)\n            Min: %4.3f / Avg: %4.3f / Max: %4.3f\n", numrows, pktloss, arr[1], sum/numrows, arr[numrows] )
}'
}
...
#	echo $ULFILE $DLFILE $PINGFILE $LOADFILE
	...
	# Start dots
	echo " Running Speed Test : $1" >>  /tmp/dots
	print_dots &
	dots_pid=$!
...
#	echo "Load PID: $load_pid"
	...
	if [ $DIRECTION = "Bidirectional" ]; then
		DL=$(summarize_speed Download $DLFILE)
		UL=$(summarize_speed Upload $ULFILE)
		echo "$DL" >> /tmp/speedtest
		echo "$UL" >> /tmp/speedtest
	else
		SP=$(summarize_speed $DIRECTION $DLFILE)
		echo "$SP" >> /tmp/speedtest
	fi

	# Summarize the ping data
	PING=$(summarize_pings $PINGFILE)
	echo "$PING" >> /tmp/speedtest

	# Summarize the load data
	LOAD=$(summarize_load $LOADFILE)
...
TH=$(uci get speedtest.test.host)
if [ ! -z $TH ]; then
	TESTHOST=$TH
fi

TESTDUR="15"
TH=$(uci get speedtest.test.duration)
if [ ! -z $TH ]; then
	TESTDUR=$TH
fi

PINGHOST="gstatic.com"
TH=$(uci get speedtest.test.ping)
if [ ! -z $TH ]; then
	PINGHOST=$TH
fi
...
MAXSESSIONS=5
TESTPROTO="-4"
TH=$(uci get speedtest.test.ipv)
if [ ! -z $TH ]; then
	TESTPROTO="-"$TH
fi

TESTSEQ=1
TH=$(uci get speedtest.test.session)
if [ $TH = "c" ]; then
	TESTSEQ=0
fi
...
echo " Last Test Date and Time : $DATE" > /tmp/speedtest
echo " " >> /tmp/speedtest
echo " Starting speedtest for $TESTDUR seconds per transfer session." >> /tmp/speedtest
echo " Test Host : $TESTHOST (IPv${TESTPROTO#-})" >> /tmp/speedtest
echo " Ping Host : $PINGHOST" >> /tmp/speedtest
if [ "$TESTSEQ " -eq "1" ]; then
	echo " Download and upload sessions are sequential." >> /tmp/speedtest
else
	echo " Download and upload sessions are concurrent." >> /tmp/speedtest
fi
echo " " >> /tmp/speedtest
