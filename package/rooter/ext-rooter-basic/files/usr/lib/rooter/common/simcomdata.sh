...
REGXa="+CPSI:[ ]*LTE,ONLINE,[0-9]\{3\}-[0-9]\{2,3\},0X[0-9A-F]\{1,5\},[0-9]\{3,9\},[0-9]\{1,3\},[-A-Z0-9]\+,[0-9]\{1,6\},[0-5],[0-5],-[0-9]\{1,3\},-[0-9]\{1,4\},"

REGXb="+CPSI:[ ]*NR5G[_ANS]*,[0-9]\{1,3\},[0-9]\{6\},-[0-9]\{1,4\},-[0-9]\{1,4\},"

REGXc="+CPSI:[ ]*NR5G[_ANS]*,ONLINE,[0-9]\{3\}-[0-9]\{2,3\},0X[0-9A-F]\{1,6\},[0-9]\{3,13\},[0-9]\{1,3\},[0-9]\{6,7\},-[0-9]\{1,4\},-[0-9]\{1,4\},"
...
if [ -n "$CPSIa" ]; then
	...
	if [ -n "$CPSIc" ]; then
		...
		CHANNEL=$(echo $CPSIc | cut -d, -f7)
		LBAND=$("$ROOTER/chan2band.sh" "$CHANNEL")
		if [ "$LBAND" = "-" ]; then
			LBAND="nxx (unknown NR5G band)"
		fi
		RSCP=$(($(echo $CPSIc | cut -d, -f8) / 10))
		ECIO=$(($(echo $CPSIc | cut -d, -f9) / 10))
		if [ "$CSQ_PER" = "-" ]; then
